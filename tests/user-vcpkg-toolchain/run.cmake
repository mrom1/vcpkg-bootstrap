...
#
# The user passes vcpkg's own toolchain (-DCMAKE_TOOLCHAIN_FILE=<vcpkg>/scripts/buildsystems/vcpkg.cmake,
# as on the command line, in a preset or from an IDE). vcpkg-bootstrap defers completely: it does not
# clone or pick another vcpkg, and reports the user's vcpkg as VCPKG_BOOTSTRAP_ROOT.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/user-vcpkg-toolchain/run.cmake

set(CASE user-vcpkg-toolchain)
set(CASE_MODES OFF)
...
# The user's own vcpkg (shallow clone is enough: manifest without builtin-baseline).
set(USER_VCPKG "${RUN_DIR}/user-vcpkg")
step(clone-vcpkg SUCCESS COMMAND git clone --depth 1 https://github.com/microsoft/vcpkg.git "${USER_VCPKG}")
if(CMAKE_HOST_WIN32)
  step(bootstrap-vcpkg SUCCESS WORKING_DIRECTORY "${USER_VCPKG}"
    COMMAND cmd /c "${USER_VCPKG}/bootstrap-vcpkg.bat" -disableMetrics)
else()
  step(bootstrap-vcpkg SUCCESS WORKING_DIRECTORY "${USER_VCPKG}"
    COMMAND sh "${USER_VCPKG}/bootstrap-vcpkg.sh" -disableMetrics)
endif()

set(BUILD "${RUN_DIR}/build")
set(TOOLCHAIN "${USER_VCPKG}/scripts/buildsystems/vcpkg.cmake")
configure(first "${CASE_DIR}" "${BUILD}" ARGS "-DCMAKE_TOOLCHAIN_FILE=${TOOLCHAIN}"
  CONTAINS "Using the vcpkg toolchain given by the user"
  NOT_CONTAINS "Using vcpkg \\(" "Cloning")
cache_get(root "${BUILD}" VCPKG_BOOTSTRAP_ROOT)
expect_same_path("VCPKG_BOOTSTRAP_ROOT" "${root}" "${USER_VCPKG}")
cache_get(own "${BUILD}" _VCPKG_BOOTSTRAP_TOOLCHAIN_FILE)
if(own_SET)
  fail("_VCPKG_BOOTSTRAP_TOOLCHAIN_FILE is set ('${own}'); the script must not set up its own toolchain")
endif()
build(first "${BUILD}")
