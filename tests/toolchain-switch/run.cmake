# toolchain-switch
#
# The vcpkg location changes in an existing build directory (VCPKG_BOOTSTRAP_ISOLATED OFF -> ON).
# CMake only reads the toolchain on the first configure, so the script must fail with "The vcpkg
# location changed" before cloning anything. Switching back works again.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/toolchain-switch/run.cmake

set(CASE toolchain-switch)
set(CASE_MODES OFF)
...
configure(shared "${CASE_DIR}" "${BUILD}" NO_ISOLATED ARGS -DVCPKG_BOOTSTRAP_ISOLATED=OFF
  CONTAINS "Using vcpkg \\(shared\\)")
root_check("${BUILD}")

configure(to-isolated "${CASE_DIR}" "${BUILD}" EXPECT FAILURE NO_ISOLATED ARGS -DVCPKG_BOOTSTRAP_ISOLATED=ON
  CONTAINS "The vcpkg location changed"
  NOT_CONTAINS "Cloning")
if(EXISTS "${BUILD}/vcpkg")
  fail("the failed switch to isolated mode created ${BUILD}/vcpkg")
endif()

configure(back "${CASE_DIR}" "${BUILD}" NO_ISOLATED ARGS -DVCPKG_BOOTSTRAP_ISOLATED=OFF
