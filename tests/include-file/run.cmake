# include-file
#
# vcpkg-bootstrap.cmake copied into a project and included with include() before project(), without
# FetchContent. Works like the FetchContent way: vcpkg in the expected place, manifest installed,
# build, run, and a second configure that reuses everything.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> [-DVB_ISOLATED=ON] -P tests/include-file/run.cmake

set(CASE include-file)
...
# The user's project: this folder's files plus a copy of the script in cmake/.
set(PROJECT_DIR "${RUN_DIR}/project")
file(COPY "${CASE_DIR}/CMakeLists.txt" "${CASE_DIR}/vcpkg.json" "${CASE_DIR}/main.cpp" DESTINATION "${PROJECT_DIR}")
file(COPY "${VB_SCRIPT_DIR}/vcpkg-bootstrap.cmake" DESTINATION "${PROJECT_DIR}/cmake")

set(BUILD "${RUN_DIR}/build")
set(ARGS "-DVCPKG_BOOTSTRAP_ISOLATED=${VB_ISOLATED}")
if(VB_SHARED_DIR)
  list(APPEND ARGS "-DVCPKG_BOOTSTRAP_INSTALL_DIR=${VB_SHARED_DIR}")
endif()
if(VB_ISOLATED)
...
# PLAIN: no FETCHCONTENT_SOURCE_DIR_VCPKG_BOOTSTRAP, the project does not use FetchContent.
configure(first "${PROJECT_DIR}" "${BUILD}" PLAIN ARGS ${ARGS} CONTAINS "${using}")
if(EXISTS "${BUILD}/_deps/vcpkg_bootstrap-src")
  fail("vcpkg-bootstrap was fetched with FetchContent; the project only includes its own copy")
endif()
root_check("${BUILD}")
...
# Second configure without VCPKG_BOOTSTRAP_ISOLATED: no clone, fetch or bootstrap, same root.
cache_get(before "${BUILD}" VCPKG_BOOTSTRAP_ROOT)
set(ARGS "")
if(VB_SHARED_DIR)
  list(APPEND ARGS "-DVCPKG_BOOTSTRAP_INSTALL_DIR=${VB_SHARED_DIR}")
endif()
configure(second "${PROJECT_DIR}" "${BUILD}" PLAIN ARGS ${ARGS}
  NOT_CONTAINS "\\[vcpkg-bootstrap\\] (Cloning|Fetching|Bootstrapping)")
cache_get(after "${BUILD}" VCPKG_BOOTSTRAP_ROOT)
if(NOT before STREQUAL after)
  fail("VCPKG_BOOTSTRAP_ROOT changed on re-configure: '${before}' -> '${after}'")
