# subproject
#
# A: a project that uses vcpkg-bootstrap is added with add_subdirectory() by a parent that does
#    not: the parent still configures and builds; the child's script and explicit install only warn.
# B: parent and child both fetch vcpkg-bootstrap: the parent's (first) one sets vcpkg up, and the
#    child's vcpkg_bootstrap_install(fmt) works.
#
...
# A: parent without vcpkg-bootstrap
set(BUILD_A "${RUN_DIR}/build-plain")
configure(parent-plain "${CASE_DIR}/parent-plain" "${BUILD_A}"
  CONTAINS "Included after project\\(parent\\)" "vcpkg is not set up for this build"
  NOT_CONTAINS "Cloning")
build(parent-plain "${BUILD_A}")
run_main(parent-plain "${BUILD_A}/child")

# B: parent and child both use vcpkg-bootstrap
set(BUILD_B "${RUN_DIR}/build-vcpkg")
configure(parent-vcpkg "${CASE_DIR}/parent-vcpkg" "${BUILD_B}"
  CONTAINS "\\[vcpkg-bootstrap\\] Installing fmt")
build(parent-vcpkg "${BUILD_B}")
run_main(parent-vcpkg "${BUILD_B}/child")
