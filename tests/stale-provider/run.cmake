# stale-provider
#
# vcpkg-bootstrap is removed from a project (USE_VCPKG=OFF) while the build directory still lists its
# provider.cmake in the cached CMAKE_PROJECT_TOP_LEVEL_INCLUDES. That stale entry must do nothing.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/stale-provider/run.cmake

set(CASE stale-provider)
set(CASE_MODES OFF)
...
# vcpkg's own VCPKG_INSTALLED_DIR: fmt must really be installed by the fallback in this build,
# not found in the shared vcpkg because another project installed it there.
set(INSTALLED "-DVCPKG_INSTALLED_DIR=${BUILD}/vcpkg_installed")

configure(on "${CASE_DIR}" "${BUILD}" ARGS -DUSE_VCPKG=ON "${INSTALLED}"
  CONTAINS "installing vcpkg port 'fmt'")
build(on "${BUILD}")
run_main(on "${BUILD}")

configure(off "${CASE_DIR}" "${BUILD}" ARGS -DUSE_VCPKG=OFF
  CONTAINS "nlohmann_json_FOUND="
  NOT_CONTAINS "installing vcpkg port" "\\[vcpkg-bootstrap\\]")
cache_get(includes "${BUILD}" CMAKE_PROJECT_TOP_LEVEL_INCLUDES)
if(NOT includes MATCHES "vcpkg-bootstrap/provider\\.cmake")
  fail("test setup: the cached CMAKE_PROJECT_TOP_LEVEL_INCLUDES no longer lists provider.cmake: '${includes}'")
endif()
file(READ "${BUILD}/vcpkg-bootstrap/install.log" install_log)   # written by step "on"
if(install_log MATCHES "nlohmann")
  fail("the stale provider installed nlohmann-json")
endif()
build(off "${BUILD}")
run_main(off "${BUILD}")
