...
#
# A: with a vcpkg.json, vcpkg_bootstrap_install() does nothing (vcpkg's manifest decides) and warns
#    about ports that are missing from the manifest.
# B: vcpkg_bootstrap_install() before project() fails with an explanation.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/install-function-manifest-mode/run.cmake

set(CASE install-function-manifest-mode)
set(CASE_MODES OFF)
...
# A: manifest mode
set(BUILD_A "${RUN_DIR}/build-manifest")
configure(manifest "${CASE_DIR}/manifest" "${BUILD_A}"
  CONTAINS "'nlohmann-json' is not listed"
  NOT_CONTAINS "'fmt' is not listed" "\\[vcpkg-bootstrap\\] Installing")
build(manifest "${BUILD_A}")
run_main(manifest "${BUILD_A}")

# B: called before project()
set(BUILD_B "${RUN_DIR}/build-before-project")
configure(before-project "${CASE_DIR}/before-project" "${BUILD_B}" EXPECT FAILURE
  CONTAINS "must be called after project\\(\\)")
