# bundled-vcpkg
#
# ENV{VCPKG_ROOT} points to a bundled vcpkg without a ports tree (like the one Visual Studio ships).
# Such a vcpkg only works with a manifest that pins a builtin-baseline (vcpkg's own rule).
# A: no vcpkg.json -> ENV{VCPKG_ROOT} is ignored with the reason, a managed vcpkg is used.
# B: vcpkg.json with builtin-baseline -> the bundled vcpkg is used.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/bundled-vcpkg/run.cmake

set(CASE bundled-vcpkg)
set(CASE_MODES OFF)
...
# Fake bundle: vcpkg-bundle.json, a toolchain stub, no ports/ directory.
set(BUNDLE "${RUN_DIR}/bundle")
file(WRITE "${BUNDLE}/vcpkg-bundle.json" "{\"readonly\": true}\n")
file(WRITE "${BUNDLE}/scripts/buildsystems/vcpkg.cmake" "message(STATUS \"fake-bundle-toolchain-loaded\")\n")
set(ENV{VCPKG_ROOT} "${BUNDLE}")

# The managed vcpkg for A lives in the run directory, independent of the user cache.
set(MANAGED "${RUN_DIR}/managed")

# A: no manifest
set(BUILD_A "${RUN_DIR}/build-no-manifest")
configure(no-manifest "${CASE_DIR}/no-manifest" "${BUILD_A}" ARGS "-DVCPKG_BOOTSTRAP_INSTALL_DIR=${MANAGED}"
  CONTAINS "Ignoring ENV\\{VCPKG_ROOT\\}.*no vcpkg.json found" "Using vcpkg \\(shared\\)"
  NOT_CONTAINS "fake-bundle-toolchain-loaded")
cache_get(root "${BUILD_A}" VCPKG_BOOTSTRAP_ROOT)
expect_same_path("VCPKG_BOOTSTRAP_ROOT" "${root}" "${MANAGED}/vcpkg")
build(no-manifest "${BUILD_A}")
run_main(no-manifest "${BUILD_A}")

# B: manifest with builtin-baseline
set(BUILD_B "${RUN_DIR}/build-with-baseline")
configure(with-baseline "${CASE_DIR}/with-baseline" "${BUILD_B}" ARGS "-DVCPKG_BOOTSTRAP_INSTALL_DIR=${MANAGED}"
  CONTAINS "is a bundled vcpkg.*using it" "fake-bundle-toolchain-loaded"
  NOT_CONTAINS "Cloning")
cache_get(root "${BUILD_B}" VCPKG_BOOTSTRAP_ROOT)
expect_same_path("VCPKG_BOOTSTRAP_ROOT" "${root}" "${BUNDLE}")
build(with-baseline "${BUILD_B}")
run_main(with-baseline "${BUILD_B}")
