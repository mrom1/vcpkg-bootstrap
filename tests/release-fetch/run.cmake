# release-fetch
#
# The published release works: FetchContent really clones vcpkg-bootstrap at the given tag from
# GitHub (no local override), and the project configures, builds and runs.
#
#   cmake -DVB_SCRIPT_TAG=v1.0.0 -P tests/release-fetch/run.cmake

set(CASE release-fetch)
set(CASE_MODES OFF)
set(REQUIRED_INPUTS VB_SCRIPT_TAG)
...
# Generate the project with the tag filled in.
set(PROJECT_DIR "${RUN_DIR}/project")
configure_file("${CASE_DIR}/CMakeLists.txt.in" "${PROJECT_DIR}/CMakeLists.txt" @ONLY)
file(COPY "${CASE_DIR}/vcpkg.json" "${CASE_DIR}/main.cpp" DESTINATION "${PROJECT_DIR}")

set(BUILD "${RUN_DIR}/build")
set(ARGS "")
if(VB_SHARED_DIR)
  set(ARGS "-DVCPKG_BOOTSTRAP_INSTALL_DIR=${VB_SHARED_DIR}")
endif()
configure(first "${PROJECT_DIR}" "${BUILD}" PLAIN ARGS ${ARGS}
  CONTAINS "Using vcpkg \\(shared\\)")
cache_get(fetched "${BUILD}" FETCHCONTENT_SOURCE_DIR_VCPKG_BOOTSTRAP)
if(fetched)
  fail("FETCHCONTENT_SOURCE_DIR_VCPKG_BOOTSTRAP is set ('${fetched}'); the release must be fetched from GitHub")
endif()
if(NOT EXISTS "${BUILD}/_deps/vcpkg_bootstrap-src/vcpkg-bootstrap.cmake")
  fail("FetchContent did not clone vcpkg-bootstrap into ${BUILD}/_deps/vcpkg_bootstrap-src")
endif()
root_check("${BUILD}")
build(first "${BUILD}")
run_main(first "${BUILD}")

message(STATUS "PASSED ${CASE} (${MODE}, ${VB_SCRIPT_TAG})")
