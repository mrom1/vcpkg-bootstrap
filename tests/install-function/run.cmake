# install-function
#
# No vcpkg.json: the project asks for ports with vcpkg_bootstrap_install(fmt nlohmann-json) after
# project(). vcpkg installs them in classic mode into <vcpkg root>/installed (vcpkg's default:
# the shared vcpkg, or <build>/vcpkg in isolated mode), and find_package() finds them.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> [-DVB_ISOLATED=ON] -P tests/install-function/run.cmake

set(CASE install-function)
...
configure(first "${CASE_DIR}" "${BUILD}"
  CONTAINS "\\[vcpkg-bootstrap\\] Installing fmt nlohmann-json")
root_check("${BUILD}")

# Classic mode installs into <vcpkg root>/installed.
cache_get(root "${BUILD}" VCPKG_BOOTSTRAP_ROOT)
cache_get(triplet "${BUILD}" VCPKG_TARGET_TRIPLET)
if(NOT triplet)
  fail("VCPKG_TARGET_TRIPLET is not in the cache")
endif()
...
endforeach()
if(EXISTS "${BUILD}/vcpkg_installed")
  fail("classic mode installed into ${BUILD}/vcpkg_installed instead of ${root}/installed")
endif()
