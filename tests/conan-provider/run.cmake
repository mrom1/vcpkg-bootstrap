# conan-provider
#
# cmake-conan's dependency provider is configured next to vcpkg-bootstrap. Only one provider can be
# active: Conan's (listed later) wins, vcpkg-bootstrap says so, and vcpkg_bootstrap_install() still
# works for what Conan does not provide.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/conan-provider/run.cmake    (needs conan 2 on PATH)

set(CASE conan-provider)
set(CASE_MODES OFF)
...
if(NOT CONAN_EXE)
  fail("conan was not found on PATH (pip install conan)")
endif()
# Conan's home inside the run directory: the user's Conan profiles and cache stay untouched.
# cmake-conan detects the default profile itself.
set(ENV{CONAN_HOME} "${RUN_DIR}/conan-home")

# cmake-conan's provider, pinned to a commit of https://github.com/conan-io/cmake-conan (develop2).
set(CMAKE_CONAN_COMMIT b1593849dd842c37ff198b9d3e7a6d4e03803121)
set(CMAKE_CONAN_SHA256 63caaf42c06620d4f6139a0e17b3442738570fb9b123585ba8247afc322f8789)
set(CONAN_PROVIDER "${RUN_DIR}/conan_provider.cmake")
file(DOWNLOAD "https://raw.githubusercontent.com/conan-io/cmake-conan/${CMAKE_CONAN_COMMIT}/conan_provider.cmake"
  "${CONAN_PROVIDER}" EXPECTED_HASH SHA256=${CMAKE_CONAN_SHA256} STATUS download_status)
list(GET download_status 0 download_code)
if(NOT download_code EQUAL 0)
  fail("downloading conan_provider.cmake failed: ${download_status}")
endif()

set(BUILD "${RUN_DIR}/build")
set(ARGS "-DCMAKE_PROJECT_TOP_LEVEL_INCLUDES=${CONAN_PROVIDER}" -DCMAKE_BUILD_TYPE=Release)
configure(first "${CASE_DIR}" "${BUILD}" ARGS ${ARGS}
  CONTAINS "Another dependency provider is configured" "CMake-Conan: first find_package\\(\\) found"
           "\\[vcpkg-bootstrap\\] Installing nlohmann-json"
  NOT_CONTAINS "not found elsewhere; installing")
cache_get(includes "${BUILD}" CMAKE_PROJECT_TOP_LEVEL_INCLUDES)
if(NOT includes MATCHES "^[^;]*/vcpkg-bootstrap/provider\\.cmake;[^;]*conan_provider\\.cmake$")
  fail("CMAKE_PROJECT_TOP_LEVEL_INCLUDES should be '<build>/vcpkg-bootstrap/provider.cmake;<conan_provider.cmake>', is '${includes}'")
endif()
build(first "${BUILD}")
run_main(first "${BUILD}")

reconfigure_check(second "${CASE_DIR}" "${BUILD}")
build(second "${BUILD}")
run_main(second "${BUILD}")
