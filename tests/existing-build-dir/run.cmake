# existing-build-dir
#
# A build directory is configured without vcpkg first, then vcpkg-bootstrap is switched on.
# CMake only reads the toolchain file on the first configure of a build directory, so the script
# must stop with a clear "delete the build directory" error - before cloning anything.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/existing-build-dir/run.cmake

set(CASE existing-build-dir)
set(CASE_MODES OFF)
...
# 1. Plain CMake project, vcpkg-bootstrap not involved.
configure(without-vcpkg "${CASE_DIR}" "${BUILD}" PLAIN ARGS -DUSE_VCPKG=OFF
  NOT_CONTAINS "\\[vcpkg-bootstrap\\]")

# 2. Same build directory, now with vcpkg-bootstrap: must fail early.
configure(with-vcpkg "${CASE_DIR}" "${BUILD}" EXPECT FAILURE ARGS -DUSE_VCPKG=ON
  CONTAINS "configured before without vcpkg's toolchain"
