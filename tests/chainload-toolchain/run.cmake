# chainload-toolchain
#
# The user already has a (non-vcpkg) toolchain, e.g. for cross-compiling. vcpkg-bootstrap keeps it
# by chainloading it through vcpkg's toolchain (VCPKG_CHAINLOAD_TOOLCHAIN_FILE).
# A: toolchain given with -DCMAKE_TOOLCHAIN_FILE.  B: toolchain given with ENV{CMAKE_TOOLCHAIN_FILE}.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/chainload-toolchain/run.cmake

set(CASE chainload-toolchain)
set(CASE_MODES OFF)
...
set(DUMMY "${CASE_DIR}/dummy-toolchain.cmake")

# A: -DCMAKE_TOOLCHAIN_FILE
set(BUILD_A "${RUN_DIR}/build-arg")
configure(arg "${CASE_DIR}" "${BUILD_A}" ARGS "-DCMAKE_TOOLCHAIN_FILE=${DUMMY}"
  CONTAINS "Chainloading toolchain: .*dummy-toolchain\\.cmake" "dummy-toolchain-loaded")
cache_get(chainload "${BUILD_A}" VCPKG_CHAINLOAD_TOOLCHAIN_FILE)
expect_same_path("VCPKG_CHAINLOAD_TOOLCHAIN_FILE" "${chainload}" "${DUMMY}")
cache_get(toolchain "${BUILD_A}" CMAKE_TOOLCHAIN_FILE)
if(NOT toolchain MATCHES "scripts/buildsystems/vcpkg\\.cmake$")
  fail("CMAKE_TOOLCHAIN_FILE is '${toolchain}', expected vcpkg's toolchain")
endif()
build(arg "${BUILD_A}")
run_main(arg "${BUILD_A}")
reconfigure_check(arg-second "${CASE_DIR}" "${BUILD_A}")
contains("dummy-toolchain-loaded")

# B: ENV{CMAKE_TOOLCHAIN_FILE}
set(ENV{CMAKE_TOOLCHAIN_FILE} "${DUMMY}")
set(BUILD_B "${RUN_DIR}/build-env")
configure(env "${CASE_DIR}" "${BUILD_B}"
  CONTAINS "Chainloading toolchain: .*dummy-toolchain\\.cmake" "dummy-toolchain-loaded")
cache_get(chainload "${BUILD_B}" VCPKG_CHAINLOAD_TOOLCHAIN_FILE)
expect_same_path("VCPKG_CHAINLOAD_TOOLCHAIN_FILE" "${chainload}" "${DUMMY}")
cache_get(toolchain "${BUILD_B}" CMAKE_TOOLCHAIN_FILE)
if(NOT toolchain MATCHES "scripts/buildsystems/vcpkg\\.cmake$")
  fail("CMAKE_TOOLCHAIN_FILE is '${toolchain}', expected vcpkg's toolchain")
endif()
build(env "${BUILD_B}")
run_main(env "${BUILD_B}")
reconfigure_check(env-second "${CASE_DIR}" "${BUILD_B}")
contains("dummy-toolchain-loaded")
