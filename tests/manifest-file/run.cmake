# manifest-file
#
# The standard use: vcpkg-bootstrap before project(), dependencies in vcpkg.json (no baseline).
# Shared mode clones vcpkg into the per-user cache, isolated mode into <build>/vcpkg (and ignores
# ENV{VCPKG_ROOT}). vcpkg installs fmt in manifest mode into <build>/vcpkg_installed. A second
# configure reuses everything: no clone, fetch or bootstrap.
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> [-DVB_ISOLATED=ON] -P tests/manifest-file/run.cmake

set(CASE manifest-file)
...
if(VB_ISOLATED)
  # Isolated mode must ignore ENV{VCPKG_ROOT}; if it did not, configure would fail on this path.
  set(ENV{VCPKG_ROOT} "${RUN_DIR}/does-not-exist")
  set(using "Using vcpkg \\(isolated\\)")
else()
  set(using "Using vcpkg \\(shared\\)")
endif()

configure(first "${CASE_DIR}" "${BUILD}" CONTAINS "${using}")
root_check("${BUILD}")
if(NOT IS_DIRECTORY "${BUILD}/vcpkg_installed")
  fail("manifest mode did not install into ${BUILD}/vcpkg_installed")
endif()
build(first "${BUILD}")
