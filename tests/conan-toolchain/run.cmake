# conan-toolchain
#
# The project gets fmt from Conan (conan install + -DCMAKE_TOOLCHAIN_FILE=conan_toolchain.cmake).
# vcpkg-bootstrap chainloads Conan's toolchain; fmt comes from Conan, and the find_package() fallback
# fills in only what Conan does not provide (nlohmann_json from vcpkg).
#
#   cmake -DVB_SCRIPT_DIR=<checkout of main> -P tests/conan-toolchain/run.cmake    (needs conan 2 on PATH)

set(CASE conan-toolchain)
set(CASE_MODES OFF)
...
set(CONAN_OUT "${RUN_DIR}/conan")
step(conan-profile SUCCESS COMMAND "${CONAN_EXE}" profile detect --force)
step(conan-install SUCCESS COMMAND "${CONAN_EXE}" install "${CASE_DIR}" "--output-folder=${CONAN_OUT}"
  --build=missing -s build_type=Release
  # No CMakeUserPresets.json next to conanfile.txt: tests never write into the repository.
  -c tools.cmake.cmaketoolchain:user_presets=)
set(CONAN_TOOLCHAIN "${CONAN_OUT}/conan_toolchain.cmake")
if(NOT EXISTS "${CONAN_TOOLCHAIN}")
  fail("conan install did not generate ${CONAN_TOOLCHAIN}")
endif()

set(BUILD "${RUN_DIR}/build")
# vcpkg's own VCPKG_INSTALLED_DIR: nlohmann-json must really be installed by the fallback in this
# build, not found in the shared vcpkg because another project installed it there.
set(ARGS "-DCMAKE_TOOLCHAIN_FILE=${CONAN_TOOLCHAIN}" -DCMAKE_BUILD_TYPE=Release
  "-DVCPKG_INSTALLED_DIR=${BUILD}/vcpkg_installed")
configure(first "${CASE_DIR}" "${BUILD}" ARGS ${ARGS}
  CONTAINS "Chainloading toolchain: .*conan_toolchain\\.cmake" "fmt_DIR=[^ ]*conan"
           "installing vcpkg port 'nlohmann-json'"
  NOT_CONTAINS "installing vcpkg port 'fmt'")
cache_get(chainload "${BUILD}" VCPKG_CHAINLOAD_TOOLCHAIN_FILE)
expect_same_path("VCPKG_CHAINLOAD_TOOLCHAIN_FILE" "${chainload}" "${CONAN_TOOLCHAIN}")
build(first "${BUILD}")
run_main(first "${BUILD}")

reconfigure_check(second "${CASE_DIR}" "${BUILD}")
contains("fmt_DIR=[^ ]*conan")
build(second "${BUILD}")
